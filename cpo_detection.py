...
def detect_cpo(btdiff_2_anom_prev, btdiff_2_anom_prev_2,
    ...
    if found_file:
        ...
        # whose BTD is below -15K be classified as CPOs'
        # And those who fulfilled this condition were classified,
        # and it was good
        btdiff_2_anom_diff = btdiff_2_anom_diff < -15

        ### PASS II ###
        # In the SECOND PASS the LORD sayeth unto the image, 'Let all
        # for whom there is an unbroken path to a generous XY gradient
        # threshold exceedance be classified as CPOs'
        # And those who fulfilled this condition were classified,
        # and it was better

        generous_gradient = total_grad > 1.5

        # Even before you do that, take all that gradient threshold
        # exceedance and add it to our CPO mask

        btdiff_2_anom_diff_orig = deepcopy(btdiff_2_anom_diff)
        btdiff_2_anom_diff[generous_gradient == 1] = 1

        # When we go through the labeled blobs they have to contain the
        # total grad AND the original btdiff anom

        # Then label the image and get all
        # connected elements
        label_objects, nb_labels = ndi.label(btdiff_2_anom_diff)
        sizes = np.bincount(label_objects.ravel())

        # Set clusters smaller than size 5 to zero
        mask_sizes = sizes > 5
        mask_sizes[0] = 0
        btdiff_2_anom_diff = mask_sizes[label_objects]

        btdiff_2_anom_diff, num = measurements.label(btdiff_2_anom_diff)

        # Then loop through each labeled blob and find if one of the
        # pixels has a gradient exceedance. If it does, the whole thing
        # is a freaking CPO
        blob_ids = np.unique(btdiff_2_anom_diff)
        blob_ids = blob_ids[blob_ids != 0]

        for i in np.arange(0, len(blob_ids)):
            target_region = btdiff_2_anom_diff == blob_ids[i]
            ...
            if 1 in btdiff_2_anom_diff_orig[target_region == 1]:
                # Next check if there is a generous gradient contained
                # within this region somewhere
                if 1 in generous_gradient[target_region == 1]:
                    pass
                else:
                    btdiff_2_anom_diff[target_region == 1] = 0
            else:
                btdiff_2_anom_diff[target_region == 1] = 0
        ...
        """
        ### PASS III ###
        # In the THIRD PASS the LORD sayeth unto the image, 'Let all
        # which are either larger than 20 pixels be classified as CPOs'
        # And those who fulfilled this condition were classified,
        ...
        m.pcolormesh(cloud_lons, cloud_lats, btdiff_2_anom_diff,
                     cmap=cmap, vmin=min, vmax=max, norm=norm)

        cbar = plt.colorbar(orientation='horizontal', fraction=0.056,
                            pad=0.06)
        cbar.ax.set_xlabel('CPO mask pass 3')
        plt.tight_layout()
        plt.savefig('CPO_mask_pass_3_' + datestrings[
            date_i] + '.png',
                    bbox_inches='tight')
        ...
        return btdiff_2_anom_diff, btdiff_2_anom_prev, btdiff_2_anom_prev_2,\
...
if __name__ == '__main__':
    ...
    for date_i in np.arange(0, len(datestrings)):
        ...
        if found_file:
            ...
            m.pcolormesh(cloud_lons, cloud_lats, total_grad,
                         cmap=cmap, vmin=min, vmax=max, norm=norm)

            cbar = plt.colorbar(orientation='horizontal', fraction=0.056,
                                pad=0.06)
            cbar.ax.set_xlabel('BT 12.0 - BT 8.7 (K) spatial gradient')
            plt.tight_layout()
            plt.savefig('BT_120_087_spatial_gradient_' + datestrings[
                date_i] + '.png',
                        bbox_inches='tight')
            ...
            btdiff_2_anom_diff[np.isnan(btdiff_2_anom_diff)] = 999
            m.pcolormesh(cloud_lons, cloud_lats, btdiff_2_anom_diff,
                         cmap=cmap, vmin=min, vmax=max, norm=norm)
            btdiff_2_anom_diff[btdiff_2_anom_diff==999] = np.nan

            cbar = plt.colorbar(orientation='horizontal', fraction=0.056,
                                pad=0.06)
            cbar.ax.set_xlabel('BT 12.0 - BT 8.7 (K) temporal gradient')
            plt.tight_layout()
            plt.savefig('BT_120_087_temporal_gradient_' + datestrings[
                date_i] + '.png',
                        bbox_inches='tight')
