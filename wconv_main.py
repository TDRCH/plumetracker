...
if __name__ == '__main__':
    ...
    for date_i in np.arange(0, len(datestrings)):
        ...
        old_cloud_bool = np.asarray([j in cloud_ids_previous for j in
                                     cloud_ids])
        old_cloud_ids = cloud_ids[old_cloud_bool]
        if len(old_cloud_bool) > 0:
            old_cloud_ids = cloud_ids[old_cloud_bool]
        else:
            old_cloud_ids = []

        # Then, for each new ID, we initialise cloud objects
        for i in np.arange(0, len(new_cloud_ids)):
            cloud = plumes.Plume(new_cloud_ids[i], datetimes[date_i])
            cloud.update_position(lats, lons, clouds, new_cloud_ids[i])
            cloud.update_duration(datetimes[date_i])
            cloud.update_bbox()
            cloud.update_majorminor_axes()
            cloud.update_area()
            cloud.update_max_extent()
            cloud.update_centroid_speed()
            cloud.update_centroid_direction()
            # cloud.update_leading_edge_4(clouds, lons, lats)
            convection_objects[str(new_cloud_ids[i])] = cloud
            print convection_objects

        # For merged IDs, we move the tracks to pre-merge tracks
        for i in np.arange(0, len(merge_cloud_ids)):
            cloud = convection_objects[str(merge_cloud_ids[i])]
            cloud.merge()

        # For old IDs, we just run an update.
        for i in np.arange(0, len(old_cloud_ids)):
            cloud = convection_objects[str(old_cloud_ids[i])]
            cloud.update_position(lats, lons, clouds, old_cloud_ids[i])
            cloud.update_duration(datetimes[date_i])
            cloud.update_bbox()
            cloud.update_majorminor_axes()
            cloud.update_area()
            ...
            cloud.update_max_extent()
            # cloud.update_leading_edge_4(clouds, lons, lats)
            convection_objects[str(old_cloud_ids[i])] = cloud

        # Clouds which no longer exist are removed and archived
        if len(cloud_ids_previous) == 0:
            removed_cloud_ids = []
        else:
            removed_cloud_bool = np.asarray([j not in cloud_ids for j in
                                       cloud_ids_previous])
            removed_cloud_ids = cloud_ids_previous[removed_cloud_bool]

        for i in np.arange(0, len(removed_cloud_ids)):
            cloud = convection_objects[str(removed_cloud_ids[i])]
            cloud.update_GPE_speed()
            cloud_archive[str(removed_cloud_ids[i])] = cloud
            del convection_objects[str(removed_cloud_ids[i])]

        clouds_previous = clouds
        cloud_ids_previous = cloud_ids
